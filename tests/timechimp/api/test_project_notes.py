...
class TestGetById:
    ...
    @pytest.mark.skipif(not TestGetAll.project_notes, reason="no project notes found")
    def test_is_dict(self):
...
class TestGetByProject:
    project_note = []
    if not TestGetAll.project_notes:
        project_note = timechimp.api.project_notes.get_by_project(project_id=TestGetById.project_note["projectId"],
                                                                  to_json=True)

    @pytest.mark.skipif(not TestGetById.project_note, reason="no project notes found")
    def test_is_list(self):
...
class TestUpdate:
    project_note = {}
    if TestGetAll.project_notes:
        project_note = timechimp.api.project_notes.update(project_note=TestGetById.project_note,
                                                          to_json=True)
